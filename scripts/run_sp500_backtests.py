...
def load_universe() -> list[dict]:
    path = Path(__file__).resolve().parents[1] / "data" / "sp500_top10.json"
    if not path.exists():
        raise FileNotFoundError("sp500_top10.json not found. Run scripts/build_sp500_top10.py first.")
...
def main() -> None:
    ...
    out = Path(__file__).resolve().parents[1] / "data" / "sp500_backtests.json"
    out.write_text(json.dumps(results, indent=2), encoding="utf-8")
    print(f"Saved S&P 500 backtests: {len(results)} runs (skipped {skipped}) -> {out}")
